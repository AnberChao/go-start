...
// StartEnv 是 Env 的具体实现
type StartEnv struct {
	folder string            // 代表.env所在的目录
	maps   map[string]string // 保存所有的环境变量
}

// NewStartEnv 有一个参数，.env文件所在的目录
// example: NewStartEnv("/envfolder/") 会读取文件: /envfolder/.env
...
func (en *StartEnv) AppEnv() string {
	return en.Get("APP_ENV")
}

// IsExist 判断一个环境变量是否有被设置
func (en *StartEnv) IsExist(key string) bool {
	_, ok := en.maps[key]
	return ok
}

// Get 获取某个环境变量，如果没有设置，返回""
func (en *StartEnv) Get(key string) string {
...
func (en *StartEnv) All() map[string]string {
	return en.maps
}
