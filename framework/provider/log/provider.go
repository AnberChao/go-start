...
import (
	...
	"github.com/JoeZhao1/go-start/framework/provider/log/formatter"
	"github.com/JoeZhao1/go-start/framework/provider/log/services"
	"io"
	"strings"
)

// StartLogServiceProvider 服务提供者
type StartLogServiceProvider struct {
...
// Register 注册一个服务实例
func (l *StartLogServiceProvider) Register(c framework.Container) framework.NewInstance {
	if l.Driver == "" {
		tcs, err := c.Make(contract.ConfigKey)
		if err != nil {
			// 默认使用console
			return services.NewStartConsoleLog
		}
	...
	switch l.Driver {
	case "single":
		return services.NewStartSingleLog
	case "rotate":
		return services.NewStartRotateLog
	case "console":
		return services.NewStartConsoleLog
	case "custom":
		return services.NewStartCustomLog
	default:
		return services.NewStartConsoleLog
	}
}
...
// Boot 启动的时候注入
func (l *StartLogServiceProvider) Boot(c framework.Container) error {
	return nil
}

// IsDefer 是否延迟加载
func (l *StartLogServiceProvider) IsDefer() bool {
	return false
}

// Params 定义要传递给实例化方法的参数
func (l *StartLogServiceProvider) Params(c framework.Container) []interface{} {
	// 获取configService
...
// Name 定义对应的服务字符串凭证
func (l *StartLogServiceProvider) Name() string {
